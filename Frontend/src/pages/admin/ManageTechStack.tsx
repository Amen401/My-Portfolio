...
export default function ManageTechStack() {
  const [logos, setLogos] = useState<any[]>([]); // Using any[] here locally to satisfy both id and _id usage in your UI
  ...
  // Helper function to normalize IDs from the backend so your UI never breaks
  const normalizeData = (item: any) => {
    const validId = item._id || item.id;
    return { ...item, id: validId, _id: validId };
  };
  ...
  const fetchLogos = async () => {
    try {
      const data = await getTechLogos(search);
      // Format the data immediately when it comes from the API
      setLogos(data.map(normalizeData));
    } catch {
      setLogos([]);
    }
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      const rawSavedLogo = await createTechLogo(newLogoData);
      const savedLogo = normalizeData(rawSavedLogo); // Normalize new item

      setLogos([...logos, savedLogo]);
      setEditingId(savedLogo.id);
      setTempLogo(savedLogo);
    } catch {
      toast("Failed to create logo. Is the backend running?", "error");
    }
  };
  ...
  const handleEdit = (logo: ITechLogo) => {
    // Safely extract the ID whether it's coming as _id (from Mongo) or id
    const currentId = (logo as any)._id || logo.id;

    if (!currentId) {
      toast("Error: Could not find logo ID", "error");
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (!editingId) {
      toast("Error: Missing ID for update", "error");
      return;
    }

    try {
      // Send the request using the safe editingId
      await updateTechLogo(editingId, tempLogo);

      setLogos(
        logos.map((l) => {
          const lId = (l as any)._id || l.id;
          return lId === editingId ? ({ ...l, ...tempLogo } as ITechLogo) : l;
        }),
      );
      setEditingId(null);
      toast("Technology updated successfully");
    } catch (error: any) {
      toast(error.response?.data?.message || "Failed to update.", "error");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (
      await confirm({
        ...
        message: "Are you sure you want to remove this technology permanently?",
        confirmTitle: "Remove",
      })
    ) {
      try {
        ...
        toast("Failed to remove. Is the backend running?", "error");
      }
    }
  };

  // UI REMAINS EXACTLY AS YOU PROVIDED IT
  return (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-neutral-500">
            Curate the technologies showcased on your homepage.
          </p>
        </div>
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          value={tempLogo.name}
                          onChange={(e) =>
                            setTempLogo({ ...tempLogo, name: e.target.value })
                          }
      ...
      <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/50 p-4 rounded-md flex items-start gap-3">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-emerald-600/80 dark:text-emerald-500/80">
            Use high-quality SVG links from{" "}
            <a
            ...
            for best results across all themes.
          </p>
        </div>
      </div>
    </div>
