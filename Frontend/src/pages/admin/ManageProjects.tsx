...
export default function ManageProjects() {
  ...
  // Function 1: Only for brand new projects
  const handleCreate = async () => {
    ...
    // Explicitly remove id for creation
    delete payload.id;
  ...
  // Function 2: Only for existing projects
  const handleUpdate = async () => {
    ...
    // Remove id from payload before sending to backend (mongoose doesn't like _id in body)
    delete payload.id;
    ...
    try {
      ...
      setProjects(
        projects.map((p) =>
          p.id === editingProject.id ? ({ ...p, ...payload } as IProject) : p,
        ),
      );
    ...
    <div className="space-y-8" id="manage-projects">
      ...
      >
        <div className="space-y-1">
          <h1 className="text-xl font-black uppercase text-neutral-900 dark:text-neutral-100 tracking-tight">
            Project Lifecycle
          </h1>
          <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">
            Database operations for engineering portfolio content.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProjects.map((p) => (
          <div
            key={p.id}
            className="bg-white dark:bg-[#171717] border border-neutral-100 dark:border-neutral-800 rounded-md p-6 space-y-4 hover:border-primary/30 transition-all flex flex-col justify-between"
          >
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-mono text-neutral-400">
                  #PRJ-{p._id.padStart(4, "0")}
                </span>
              ...
              <div className="space-y-1">
                <h3 className="text-xs font-black text-neutral-900 dark:text-neutral-100 uppercase tracking-tight line-clamp-1">
                  {p.title}
                </h3>
              ...
              <div className="flex flex-wrap gap-1 pt-2">
                {p.techStack.map((t, idx) => (
                  <span
                    key={`${t}-${idx}`}
                    className="text-[8px] font-black bg-neutral-100 dark:bg-neutral-800 text-neutral-500 px-2 py-0.5 rounded-md uppercase tracking-tighter"
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="flex items-center justify-between pt-6 border-t border-neutral-50 dark:border-neutral-800/50 mt-auto">
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-2 text-neutral-400 hover:text-primary hover:bg-primary/10 rounded-md transition-all cursor-pointer"
                >
                  <Edit3 className="w-4 h-4" />
                </button>
                <button
                  onClick={() => handleDelete(p._id)}
                  className="p-2 text-neutral-400 hover:text-red-500 hover:bg-red-500/10 rounded-md transition-all cursor-pointer"
                >
              ...
              <a
                ...
                className="p-2 text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors cursor-pointer text-sm font-medium"
              >
      ...
      >
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-neutral-400 flex items-center gap-1.5">
                <LayoutGrid className="w-3 h-3" /> Project Architecture Title
              </label>
              <input
                type="text"
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-neutral-400 flex items-center gap-1.5">
                <Info className="w-3 h-3" /> Project Narrative
              </label>
              <textarea
                rows={3}
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-neutral-400 flex items-center gap-1.5">
              <Link className="w-3 h-3 cursor-pointer" /> Tech Stack (Comma
              Separated)
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-neutral-400 flex items-center gap-1.5">
                <Globe className="w-3 h-3" /> Live Demo Link
              </label>
              <input
                type="text"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-neutral-400 flex items-center gap-1.5">
                <Link className="w-3 h-3 cursor-pointer" /> Source Code
                Repository
              </label>
              <input
                type="text"
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button
              onClick={editingProject?._id ? handleUpdate : handleCreate}
