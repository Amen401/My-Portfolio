  ...
  if (!project)
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Link
          to="/projects"
          className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary hover:text-primary/80 transition-colors cursor-pointer"
        >
          Back to Projects
        </Link>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {project.techStack.map((tech, idx) => (
              <span
                key={`${tech}-${idx}`}
                className="text-[10px] font-bold uppercase tracking-wider bg-neutral-100 dark:bg-neutral-900 px-3 py-1 rounded-full text-neutral-600 dark:text-neutral-400"
              >
                {tech}
              </span>
            ))}
        ...
        <div className="flex items-center gap-4 pt-4 border-t border-neutral-100 dark:border-neutral-800">
          {project.githubLink && (
            <a
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-md bg-black text-white hover:bg-neutral-800 transition-all cursor-pointer text-sm font-medium"
            >
          ...
          {project.liveLink && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-md border border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-all cursor-pointer text-sm font-medium text-neutral-900 dark:text-neutral-100"
            >
      ...
      <section className="space-y-6" id="project-features">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {project.features.map((feature, i) => (
            <div
              key={i}
              className="flex gap-4 p-4 rounded-md bg-white dark:bg-neutral-900/40 border border-neutral-50 dark:border-neutral-800/50"
            >
